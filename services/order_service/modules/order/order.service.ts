...
export class OrderService {
  ...
  try {
    // Create order in DB, now including packageType
    const order = await Order.create({
      ...
      billingDetails,
      status: 'pending',
      packageType: package_type,  // <-- store here
    });

    // Create Razorpay order via payment service (unchanged)
